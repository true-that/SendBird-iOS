...
class GroupChannelChattingViewController: UIViewController, SBDConnectionDelegate, SBDChannelDelegate, ChattingViewDelegate, MessageDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, ReactionDetectionDelegate {
  ...
  private var detectionModule: AffectivaReactionDetectionModule?
  ...
  override func viewDidLoad() {
    super.viewDidLoad()
      ...
      // Simulator
      chattingView.takePhotoButton.isHidden = true
    #else
      detectionModule = AffectivaReactionDetectionModule()
    #endif
  ...
  @objc private func openMoreMenu() {
    ...
    let seeMemberListAction = UIAlertAction(title: Bundle.truedatLocalizedStringForKey(key: "SeeMemberListButton"), style: UIAlertActionStyle.default) { action in
      DispatchQueue.main.async {
    ...
    let inviteUserListAction = UIAlertAction(title: Bundle.truedatLocalizedStringForKey(key: "InviteUserButton"), style: UIAlertActionStyle.default) { action in
      DispatchQueue.main.async {
  ...
  private func loadPreviousMessage(initial: Bool) {
    ...
    self.groupChannel.getPreviousMessages(byTimestamp: timestamp, limit: 30, reverse: !initial, messageType: SBDMessageTypeFilter.all, customType: "") { messages, error in
      ...
      if initial {
        ...
        var shouldSetDetectionDelegate = false
        for item in messages! {
          ...
          if message is SBDUserMessage || message is SBDFileMessage {
            let senderId = message is SBDUserMessage ? (message as! SBDUserMessage).sender!.userId : (message as! SBDFileMessage).sender!.userId
            if senderId != SBDMain.getCurrentUser()?.userId {
              shouldSetDetectionDelegate = true
            } else if message is SBDUserMessage &&
              (message as! SBDUserMessage).customType == ChattingView.reactionMessageType {
              shouldSetDetectionDelegate = false
            }
          }
        }
        if shouldSetDetectionDelegate {
          self.detectionModule?.delegate = self
        }
  ...
  private func sendMessageWithReplacement(replacement: OutgoingGeneralUrlPreviewTempModel) {
    let preSendMessage = self.groupChannel.sendUserMessage(replacement.message, data: "", customType: "", targetLanguages: ["ar", "de", "fr", "nl", "ja", "ko", "pt", "he", "es", "zh-CHS"]) { userMessage, error in
  ...
  fileprivate func send(message: String, customType: String = "") {
    do {
      let detector: NSDataDetector = try NSDataDetector(types: NSTextCheckingResult.CheckingType.link.rawValue)
      let matches = detector.matches(in: message, options: NSRegularExpression.MatchingOptions(rawValue: 0), range: NSMakeRange(0, message.characters.count))
      var url: URL?
      for item in matches {
    ...
    let preSendMessage = self.groupChannel.sendUserMessage(message, data: "", customType: customType, targetLanguages: ["ar", "de", "fr", "nl", "ja", "he", "ko", "pt", "es", "zh-CHS"], completionHandler: { userMessage, error in
      DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + .milliseconds(150), execute: {
        let preSendMessage = self.chattingView.preSendMessages[(userMessage?.requestId)!] as! SBDUserMessage
        self.chattingView.preSendMessages.removeValue(forKey: (userMessage?.requestId)!)
  ...
  @objc private func didTapSend() {
    if self.chattingView.messageTextView.text.characters.count > 0 {
      ...
      send(message: chattingView.messageTextView.text)
      chattingView.messageTextView.text = ""
    }
  }
  ...
  func didStartReconnection() {
    if self.navItem.titleView != nil && self.navItem.titleView is UILabel {
      DispatchQueue.main.async {
        (self.navItem.titleView as! UILabel).attributedText = Utils.generateNavigationTitle(mainTitle: String(format: Bundle.truedatLocalizedStringForKey(key: "GroupChannelTitle"), self.groupChannel.memberCount), subTitle: Bundle.truedatLocalizedStringForKey(key: "ReconnectingSubTitle"))
      }
    }
  }
  ...
  func didSucceedReconnection() {
    ...
    self.groupChannel.refresh { error in
      if error == nil {
        DispatchQueue.main.async {
          if self.navItem.titleView != nil && self.navItem.titleView is UILabel {
            (self.navItem.titleView as! UILabel).attributedText = Utils.generateNavigationTitle(mainTitle: String(format: Bundle.truedatLocalizedStringForKey(key: "GroupChannelTitle"), self.groupChannel.memberCount), subTitle: Bundle.truedatLocalizedStringForKey(key: "ReconnectedSubTitle"))
          }

          DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + .seconds(1)) {
            if self.navItem.titleView != nil && self.navItem.titleView is UILabel {
              (self.navItem.titleView as! UILabel).attributedText = Utils.generateNavigationTitle(mainTitle: String(format: Bundle.truedatLocalizedStringForKey(key: "GroupChannelTitle"), self.groupChannel.memberCount), subTitle: "")
            }
          }
        }
      }
  ...
  func didFailReconnection() {
    if self.navItem.titleView != nil && self.navItem.titleView is UILabel {
      DispatchQueue.main.async {
        (self.navItem.titleView as! UILabel).attributedText = Utils.generateNavigationTitle(mainTitle: String(format: Bundle.truedatLocalizedStringForKey(key: "GroupChannelTitle"), self.groupChannel.memberCount), subTitle: Bundle.truedatLocalizedStringForKey(key: "ReconnectionFailedSubTitle"))
      }
    }
  }
    ...
    if sender == self.groupChannel {
      if sender.getTypingMembers()?.count == 0 {
        ...
        if sender.getTypingMembers()?.count == 1 {
          self.chattingView.startTypingIndicator(text: String(format: Bundle.truedatLocalizedStringForKey(key: "TypingMessageSingular"), (sender.getTypingMembers()?[0].nickname)!))
        } else {
          self.chattingView.startTypingIndicator(text: Bundle.truedatLocalizedStringForKey(key: "TypingMessagePlural"))
        }
      }
    }
  }
  ...
  func channel(_ sender: SBDGroupChannel, userDidJoin user: SBDUser) {
    if self.navItem.titleView != nil && self.navItem.titleView is UILabel {
      DispatchQueue.main.async {
        (self.navItem.titleView as! UILabel).attributedText = Utils.generateNavigationTitle(mainTitle: String(format: Bundle.truedatLocalizedStringForKey(key: "GroupChannelTitle"), self.groupChannel.memberCount), subTitle: nil)
      }
    }
  }
  ...
  func channelWasChanged(_ sender: SBDBaseChannel) {
    if sender == self.groupChannel {
      DispatchQueue.main.async {
        self.navItem.title = String(format: Bundle.truedatLocalizedStringForKey(key: "GroupChannelTitle"), self.groupChannel.memberCount)
      }
    }
  }
  ...
  func clickProfileImage(viewCell: UITableViewCell, user: SBDUser) {
    ...
    let seeBlockUserAction = UIAlertAction(title: Bundle.truedatLocalizedStringForKey(key: "BlockUserButton"), style: UIAlertActionStyle.default) { action in
  ...
  func clickMessage(view: UIView, message: SBDBaseMessage) {
    ...
    if message is SBDUserMessage {
      ...
      if userMessage.customType != nil && userMessage.customType == "url_preview" {
        ...
        do {
          let previewData = try JSONSerialization.jsonObject(with: data, options: JSONSerialization.ReadingOptions(rawValue: 0))
  ...
  func clickResend(view: UIView, message: SBDBaseMessage) {
    ...
    let resendAction = UIAlertAction(title: Bundle.truedatLocalizedStringForKey(key: "ResendFailedMessageButton"), style: UIAlertActionStyle.default) { action in
      if message is SBDUserMessage {
        ...
        do {
          let detector: NSDataDetector = try NSDataDetector(types: NSTextCheckingResult.CheckingType.link.rawValue)
          let matches = detector.matches(in: resendableUserMessage.message!, options: NSRegularExpression.MatchingOptions(rawValue: 0), range: NSMakeRange(0, (resendableUserMessage.message!.characters.count)))
          var url: URL?
          for item in matches {
            let match = item as NSTextCheckingResult
            url = match.url
  ...
  fileprivate func handleGif(_ imageData: Data, _ imageName: NSString, _ mimeType: CFString?) {
    /***********************************/
  ...
  fileprivate func handleStaticImage(_ imageData: Data, _ imageName: NSString, _ mimeType: CFString?) {
    /***********************************/
    /* Thumbnail is a premium feature. */
    /***********************************/
    let thumbnailSize = SBDThumbnailSize.make(withMaxWidth: 320.0, maxHeight: 320.0)
...
//        }
        ...
        if (mimeType! as String) == "image/gif" {
          ...
          self.handleStaticImage(imageData, imageName, mimeType)
        }
